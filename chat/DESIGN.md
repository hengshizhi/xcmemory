...
使用 `<monologue>...</monologue>` + `<reply>...</reply>` 标签。解析清晰，LLM 遵循格式能力强。

### ✅ 2. 记忆上下文注入时机：对话前轻检索 + 自白中深检索

- 对话前：用用户输入轻度检索（top 3），注入 system prompt
- 自白中：触发「回忆一下」时深度检索（top 5），注入自白

### ✅ 3. 记忆触发粒度：提取触发词后内容

提取「回忆一下」/「记住」后面的内容作为 NL 查询文本，查询更精确。

### ✅ 4. holder 映射

角色的 `name` 自动映射为星辰记忆的 `holder`，NL 查询中的"我"会被正确映射。
