...
**Text2Mem** Facets（四字段：subject/time/location/topic）
...
```
<time><subject><action><object><purpose><result>_<location>
```
...
**① time 槽——只能是单一时间词**，subject 不能混入 time 槽：
...
- 格式：INSERT INTO memories VALUES ('<time><subject><action><object><purpose><result>', 'description', lifecycle)
- 六槽必须等长，缺槽用 <无> 占位
- action 只用预定义动词：<是>/<与>/<的>/<同意>/<拒绝>/<希望>/<遵循>/<发生于>/<发生>/<想>/<说>/<做>...
- time 只用：<平时>/<少年期>/<童年>/<那天晚上>/<深夜>/<早上>/<本周早些时候>/<YYYY-MM-DD>
- lifecycle 规则：<平时>/<少年期>/<童年> → 999999；<本周早些时候> → 604800；<那天晚上>/<深夜>/<早上> → 86400
- 单一事实：一条记忆只表达一个独立事实
- description 不重复六槽已有信息
```
