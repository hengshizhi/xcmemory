...
- 拆出的陈述句应**尽量契合六槽位**的表达能力：
  ...
  - **每个槽位只承载一个词，不写短句**
  - scene 槽：时间场景（平时/晚上/周末/假期/早上/深夜等）或空间场景（家里/公司/学校/户外/线上/路上等），信息中有场景线索就写，没有就留待后续填充。禁止用 <所有>
  - subject 槽：核心角色
...
- **scene**：时间或空间场景——事件的情境背景。有就写，没有用 <无>。
  ★禁止使用 <所有>★
  时间示例：<平时>/<少年期>/<童年>/<深夜>/<早上>/<晚上>/<白天>/<周末>/<假日>/<YYYY-MM-DD>
  空间示例：<家里>/<公司>/<学校>/<户外>/<线上>/<路上>/<日本>/<东京>（任意地点均可）
  社会场景：<和哥哥在一起时>
  无场景信息时：<无>
...
2. "星织的名字是星织"
   - scene=<无>, subject=<星织>, action=<的>, object=<名字>, purpose=<名字>, result=<星织>

3. "星织有个哥哥叫绯绯"
   - scene=<无>, subject=<星织>, action=<有>, object=<哥哥>, purpose=<关系>, result=<绯绯>

4. "星织和绯绯只差一岁"
   - scene=<无>, subject=<星织>, action=<差>, object=<绯绯>, purpose=<年龄差距>, result=<一岁>
...
12. "星织的密码是abc123"
    - scene=<无>, subject=<星织>, action=<的>, object=<密码>, purpose=<密码>, result=<abc123>
...
## ★ 每个槽位只填一个词或一个概念，严禁填短句/短语 ★

  这是最重要的规则。每个槽位是关键词索引，不是叙述文本。

  ❌ 错误示例（槽位塞了短句）：
    action=<被父亲交给哥哥照顾>    → 短句！应该拆
    object=<和绯绯一起看电影>      → 短语！
    object=<发展恋人关系>          → 短语！
    purpose=<我是谁>               → 短句！
    result=<不知道自己是旅行者>    → 短句！
  ...
  ✅ 正确做法：把多词信息拆分到不同槽位：
    短的 action/result 用槽位装，长的叙述内容放到 VALUES 第二个参数（content）里

## 各槽含义
- **scene**：时间/空间/社会场景——事件发生的情境背景。有场景就写，没有就用 <无>。
  ★禁止使用 <所有>！<所有> 不是场景，是扔掉了所有场景信息★
  空间场景示例：<家里><公司><学校><户外><日本><沃尔玛><东京>
  时间场景示例：<平时><少年期><深夜><早上><周末><2026-03-15>
  社会场景示例：<和哥哥在一起时><和绯绯约会时>
  无场景信息时：<无>（不是 <所有>）
- **subject**：action 的发出者——谁执行了这个动作？可以是「{holder}」、也可以是其他任何人或实体。陈述句中说"我"时映射为「{holder}」，陈述句说的是其他人就以那个人的名为 subject。一个名字即可，不要加头衔
- **action**：主体施加于客体的关系/动作。预定义列表（每个单字）：
  <是><有><与><的><叫><差><来自><喜欢><知道><不知道><想><说><做><同意><拒绝><希望><遵循><发生><发生于>
  无法匹配时用最接近的单字动词。一个单字，不要多字短语
- **object**：action 的承受者或关联对象。一个词或概念
- **★purpose★**：本条记忆描述的**语义类别**——在回答什么类型的问题？如 <名字>、<身份>、<年龄差距>、<关系>、<喜好>、<经历>、<密码>、<技能>。这是 WHERE 条件的核心命中维度。无明确类别时填 <无>
- **★result★**：上述类别的**具体值或结论**。如 <星织>、<旅行者>、<一岁>、<哥哥>、<火锅>、<abc123>。是对应 purpose 问题的答案。一个词或值。无结论时填 <无>
...
3. **purpose 和 result 拆开写，不要合并到 object**
   - ❌ object=<名字星织>  → 把类别和值混在一起
   - ✅ object=<名字>, purpose=<名字>, result=<星织>
4. **每个槽位一个词/一个概念，绝不放短句**——多词信息拆到不同槽位或 content 里

# ★★ 范例 ★★

陈述句："星织的名字是星织"（无场景线索）
→ INSERT INTO memories VALUES ('<无><星织><的><名字><名字><星织>', '星织的名字是星织', {reference_duration})
  解读：无场景→<无>。主体星织-拥有-名字，这条记忆解答「名字」问题，答案是「星织」
...
陈述句："星织和绯绯只差一岁"
→ INSERT INTO memories VALUES ('<无><星织><差><绯绯><年龄差距><一岁>', '星织和绯绯只差一岁', {reference_duration})
  解读：年龄差距是属性关系，无特定场景→<无>

陈述句："星织有个哥哥叫绯绯"
→ INSERT INTO memories VALUES ('<无><星织><有><哥哥><关系><绯绯>', '星织有个哥哥叫绯绯', {reference_duration})
  解读：关系事实，无特定场景→<无>

陈述句："星织的密码是abc123"
→ INSERT INTO memories VALUES ('<无><星织><的><密码><密码><abc123>', '星织的密码是abc123', {reference_duration})

陈述句："星织喜欢吃火锅"（有场景线索：平时/日常）
→ INSERT INTO memories VALUES ('<平时><星织><喜欢><火锅><喜好><火锅>', '星织喜欢吃火锅', {reference_duration})

陈述句："星织明天要开会"（有场景线索：明天）
→ INSERT INTO memories VALUES ('<明天><星织><做><开会><计划><开会>', '星织明天要开会', {reference_duration})

陈述句："母亲在日本是战争的受害者"（有场景线索：日本）
→ INSERT INTO memories VALUES ('<日本><母亲><是><受害者><经历><战争>', '母亲在日本是战争的受害者', {reference_duration})
  解读：scene=<日本>（事件地点），subject=<母亲>（主体是母亲，不是星织）

陈述句："早上妈妈出门买菜"
→ INSERT INTO memories VALUES ('<早上><妈妈><做><买菜><活动><买菜>', '早上妈妈出门买菜', {reference_duration})
  解读：scene=<早上>（时间），subject=<妈妈>（主体），没有用 <所有>

# ★★ 反例（不要这样写）★★

❌ scene=<所有> → 禁止！<所有> 不是场景
❌ object=<和绯绯一起看电影> → 短语！拆成 object=<电影>, result=<绯绯>
❌ scene=<不知道什么时候和哥哥> → 短句！写成 <无> 或 <和哥哥在一起时>
❌ 输出缺少 VALUES 或 content 或 lifecycle → 必须三部分完整
...
规则：
- 必须用 <mql>...</mql> 包裹
- 每条 INSERT 包含完整的三个参数：'(六槽句)', '内容', lifecycle秒数
- 多条用分号分隔，每一条都独立完整
- 输入有几条陈述句就输出几条 INSERT，不许少
